...
- **`ADVANCED_CONSENT` in `site.js` is a legal decision, not a technical one.** It is `false` deliberately. Flipping it to `true` makes GA4 load before consent in cookieless mode, which recovers most of the traffic currently invisible, but sends data about non-consenting visitors to Google. James has taken it to legal review; see `Website assets/Consent mode - note for legal review.md`. **Do not flip it without his say-so, and update the privacy notice first** — it currently states analytics load "only if you accept", which advanced mode would make untrue.
- **The banner deliberately keeps reject more prominent than accept** (10.7:1 against 4.9:1). Offered to rebalance it; James chose to leave it. Not an oversight.
- **Do not debug Clarity in an automation browser: it blocks the script tag.** The tell is a resource-timing entry with `responseStatus: 0` and `transferSize: 0` while `fetch()` of the exact same URL returns 200 with a body. `onload` still fires, so the script looks loaded when nothing ran. Verified 15 Sep 2026: the site's implementation and the Clarity project are both fine, and hours can be lost to this. To test Clarity for real, `eval()` the bootstrap from `https://www.clarity.ms/tag/<id>` in an isolated frame — done that way it reaches `clarity.v = 0.8.69` and uploads to `a.clarity.ms/collect`.
- **`"track": false` in the Clarity tag config is not the off switch.** It appears in this project's config and recording still works, so it is not evidence of a problem. Likely relates to Bing UET ad tracking rather than session recording.
- **Both GA4 and Clarity are consent-gated, so neither sees a visitor who ignores the cookie banner.** That is correct under PECR, but it means low analytics volume is often a consent-rate story rather than a bug. Check the acceptance rate before suspecting the code.
